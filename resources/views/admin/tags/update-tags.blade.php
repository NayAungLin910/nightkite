...
@section('meta-description', "The tag, $tag->title can be renamed on the current page.")

@section('meta-og-title', "$tag->title, Rename Page - NightKite")
@section('meta-og-description',
    "$tag->title, tag can be easily renamed by just submitting the form on the current
    page.")

@section('custom-content')
    <div class="mx-2 mt-2">
        <h1 class="text-xl text-center mt-[4rem] lg:mt-0 mb-10"><i class="fa-solid fa-pen-to-square mr-1"></i> Rename the tag,
            {{ $tag->title }}</h1>
        @if ($errors->any())
        ...
        <form action="{{ route('admin.dashboard.update-tag', ['slug' => $tag->slug]) }}" method="POST">
            <div class="mb-4 place-content-center items-center px-4 flex gap-2">
                ...
                <div class="my-2">
                    ...
                    <input type="text" value="{{ $tag->title }}" placeholder="Tag's name"
                        class="input-form-sky md:w-80" id="tag-name" name="name" />
                </div>
